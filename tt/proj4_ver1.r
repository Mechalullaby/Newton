...
# In case not converged, if H is not positive definite, perturb it to be so
...
# After halvings, theta=theta+delta, and func, grad, H is updated, and move
# to next ieration.
...
newt <- function(theta,func,grad,hess=NULL,...,tol=1e-8,fscale=1,
  ...
  if (is.null(hess)){ # If Hessian is not provided in newt, namely hess=NULL
    hess <- null.hess  # calling null.hess
  }
  H <- hess(theta, ...)
